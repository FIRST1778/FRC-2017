...
public class MotionProfilePrototype {
	 ...
	 * keep one copy.
	 */
	private static CANTalon.MotionProfileStatus _status = new CANTalon.MotionProfileStatus();
	 ...
	 * motion profile.
	 */
	private static CANTalon _talon;
	/**
	 ...
	 */
	private static CANTalon.SetValueMotionProfile _setValue = CANTalon.SetValueMotionProfile.Disable;
	/**
	 ...
	 * every 10ms.
	 */
	static class PeriodicRunnable implements java.lang.Runnable {
	    public void run() {  _talon.processMotionProfileBuffer();    }
	}
	private static Notifier _notifier;
	
	private static boolean _bFin = false;
	private static int _timeouts = 0;
	
	private static int motionProfileIndex = 0;
			...
			switch (_state) {
				...
				case 2: /* check the status of the MP */
					...
					if (_status.activePointValid && _status.activePoint.isLastPoint) {
						...
						_loopTimeout = -1;
						_bFin= true;
						System.out.println("motion path complete");
					}
					break;
			}
		}
	...
	private static void startFilling() {
		...
		startFilling(GeneratedMotionProfile.Points[motionProfileIndex],GeneratedMotionProfile.kNumPoints[motionProfileIndex]);
	}

	private static void startFilling(double[][] profile, int totalCnt) {
...
/**********************************************/
	public static void autoInit() {
        //_talon.setPosition(0);	      // initializes encoder to zero - note: causes herky-jerky behavior	        	
	}
	...
	public static void moveProfile(int i) {
		
        System.out.println("Starting motor profile " + i + "...");
		motionProfileIndex = i;
		startMotionProfile();
	}
	
	public static void disabledInit() {
        System.out.println("resetting motor profile...");
        reset();
	}
